...
def calculate_score():
    """Calculate the overall green building score from questionnaire"""
    if not st.session_state.questionnaire_completed:
        return 0

    yes_count = sum(1 for answer in st.session_state.questionnaire_answers.values() if answer)
    ...
    score = min(100, round((yes_count / total_questions) * 100, 1))
